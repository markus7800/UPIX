...
print(batched_vmap(lambda x, y: jnp.sum(x) + y, BATCH_SIZE, in_axes=(SUM_AXIS,None), out_axes=0)(X, jnp.array(1,int)))
print(batched_vmap(lambda x, y: (jnp.sum(x), y), BATCH_SIZE, in_axes=(SUM_AXIS,None), out_axes=(0,None))(X, jnp.array(1,int)))