...
plt.show()
# %%
x_sample = mixture_sample(jax.random.PRNGKey(0), 1_000_000)
...
#%%
x_mh_sample = mh_kernel(x_sample, jax.random.split(jax.random.PRNGKey(0), x_sample.size))
...
plt.hist(x_sample, density=True, bins=100, alpha=0.5, color="tab:blue")
plt.plot(xs, ps, color="tab:blue")
q_sample = Q_sample(jax.random.PRNGKey(0), 1_000_000)
...
N = 100
q_mh_sample = apply_mh_kernel_n(q_sample, jax.random.PRNGKey(0), N, mh_kernel)
...
qs_mh = est_q_mh_log_prob(xs, N, mh_kernel, jax.random.PRNGKey(0), 1_000, Q_log, mixture_log_P)

plt.hist(q_mh_sample, density=True, bins=100, alpha=0.5, color="tab:blue")
plt.plot(xs, qs_mh, color="tab:blue")
plt.show()
# %%
qs_mh = est_q_mh_log_prob(xs, 1, mh_kernel, jax.random.PRNGKey(0), 1_000, mixture_log_P, mixture_log_P)
...
# estimate z
T = 1_000
posterior_approx = mixture_sample(jax.random.PRNGKey(0), T)
...
def get_IS_log_Q(posterior_approx: jax.Array, mix: bool):
    ...
    return log_Q
# %%
is_q_sample = IS_Q_sample(posterior_approx, jax.random.PRNGKey(0))
...
# %%
M = 100
is_q_sample = jax.vmap(IS_Q_sample, (None,0))(posterior_approx, jax.random.split(jax.random.PRNGKey(0),M))
log_qs = jax.vmap(get_IS_log_Q(posterior_approx, True))(is_q_sample)
jax.lax.exp(jax.scipy.special.logsumexp(mixture_log_P(is_q_sample) - log_qs)) / is_q_sample.size
# %%
is_q_mh_sample = mh_kernel(IS_Q_sample(posterior_approx, jax.random.PRNGKey(0)), jax.random.split(jax.random.PRNGKey(0),T))
is_q_mh_sample_0 = mh_kernel(is_q_mh_sample, jax.random.split(jax.random.PRNGKey(0),T))
