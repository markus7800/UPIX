...
# uv run --frozen -p python3.13 --extra=cpu evaluation/pedestrian/viz_mcmc_scale.py sequential pmap -vmap_batch_size 524288 -maxpow ${1:-20} -host_device_count 8 --no_progress

# uv run --frozen -p python3.13 --extra=cpu --with=pandas evaluation/gp/viz_vi_elbo_scale.py sequential smap_local -maxpow ${2:-14} -host_device_count 8 --no_progress

# uv run --frozen -p python3.13 --extra=cpu --with=pandas evaluation/gp/viz_smc_particle_scale.py sequential smap_local -maxpow ${3:-15} -host_device_count 8 --no_progress

uv run --frozen -p python3.13 --extra=cpu evaluation/gmm/viz_mcmc_scale.py sequential pmap -vmap_batch_size 524288 -maxpow ${4:-18} -host_device_count 8 --no_progress
