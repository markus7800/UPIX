...
@dist function negbeta(alpha, beta)
    0-Gen.beta(alpha,beta)
end
u2_dist = Gen.HeterogeneousMixture([negbeta, beta])
...
@gen function split_randomness(tr)
    ...
    u1 ~ beta(2, 2)
    # u2 ~ beta(2, 2) # original
    u2 ~ u2_dist([0.5,0.5],2,2,2,2)
    u3 ~ beta(1, 1)
    ...
    # println(w, ", ", σ², ", ", μ, ", ", w1, ", ", μ1, ", ", σ1², ", ", w2, ", ", μ2, ", ", σ2²)
...
@transform split_transform (tr_in, ch_in) to (tr_out, ch_out) begin
    ...
    μ1, μ2 = μ - u2 * sqrt(σ² * w2/w1), μ + u2 * sqrt(σ² * w1/w2)
    σ1², σ2² = u3 * (1 - u2^2) * σ² * w/w1, (1 - u3) * (1 - u2^2) * σ² * w/w2
    # println("j_star=$j_star j1=$j1 j2=$j2")
    @write(tr_out[:k], k + 1, :disc)
    for i=1:n
        z = @read(tr_in[:z => i], :disc)
        if z == j_star
